...
def modeling():
    ...
    for tollgate_id in tollgate_list:
        ...
        def divide_by_time_slot(data_df, train=True):
            ...
            # 按天拆分成无数个元素
            train_lst_6_8 = [train_6_8[(train_6_8["day"] == i) & (train_6_8["month"] == 9)]["volume"] for i in
                              range(19, 31, 1)]
            train_lst_6_8 += [train_6_8[(train_6_8["day"] == i) & (train_6_8["month"] == 10)]["volume"] for i in
                               range(1, 18, 1)]
            predict_lst_6_8 = []
            if (train):
                # 如果是训练集，还得做8点到10点的数据
                ...
                predict_lst_6_8 += [train_8_10[(train_8_10["day"] == i) & (train_8_10["month"] == 9)]["volume"] for i in
                                range(19, 31, 1)]
                predict_lst_6_8 += [train_8_10[(train_8_10["day"] == i) & (train_8_10["month"] == 10)]["volume"] for i in
                                 range(1, 18, 1)]
            ...
            if (train):
                ...
                train_17_19.index = data_df[(data_df["hour"] >= 8) & (data_df["hour"] < 10)]["time"]
